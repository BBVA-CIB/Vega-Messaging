...
public class SubcribersPollerTest
{
    ...
    public void createPollerAndPoll() throws Exception
    {
        ...
        // Send an unexpected type message
        IPC_PUBLISHER.sendMessage((byte)66, topicId1, sendBuffer, 0, 4);

        // Send IPD messages
        sendBuffer.putInt(0, 11);
        IPC_PUBLISHER.sendMessage(MsgType.DATA, topicId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 12);
        IPC_PUBLISHER.sendRequest(MsgType.DATA_REQ, topicId1, requestId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 13);
        IPC_PUBLISHER.sendResponse(responseId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 14);
        IPC_PUBLISHER.sendMessage(MsgType.ENCRYPTED_DATA, topicId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 15);
        IPC_PUBLISHER.sendRequest(MsgType.HEARTBEAT_REQ, topicId1, heartbeatRequestId1, sendBuffer, 0, 4);
        ...
        UCAST_PUBLISHER.sendMessage(MsgType.DATA, topicId2, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 22);
        UCAST_PUBLISHER.sendRequest(MsgType.DATA_REQ, topicId2, requestId2, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 23);
        UCAST_PUBLISHER.sendResponse(responseId2, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 24);
        UCAST_PUBLISHER.sendMessage(MsgType.ENCRYPTED_DATA, topicId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 25);
        UCAST_PUBLISHER.sendRequest(MsgType.HEARTBEAT_REQ, topicId1, heartbeatRequestId2, sendBuffer, 0, 4);
        ...
        MCAST_PUBLISHER.sendMessage(MsgType.DATA, topicId3, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 32);
        MCAST_PUBLISHER.sendRequest(MsgType.DATA_REQ, topicId3, requestId3, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 33);
        MCAST_PUBLISHER.sendResponse(responseId3, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 34);
        MCAST_PUBLISHER.sendMessage(MsgType.ENCRYPTED_DATA, topicId1, sendBuffer, 0, 4);
        sendBuffer.putInt(0, 35);
        MCAST_PUBLISHER.sendRequest(MsgType.HEARTBEAT_REQ, topicId1, heartbeatRequestId3, sendBuffer, 0, 4);

        // Wait for messages to arrive
        Thread.sleep(1000);

        // Check the messages
        Assert.assertTrue(listener.rcvMessages.contains(11));
        Assert.assertTrue(listener.rcvMessages.contains(21));
        Assert.assertTrue(listener.rcvMessages.contains(31));

        // Check the encrypted messages
        Assert.assertTrue(listener.rcvEncryptedMessagesCount.get() == 3);

        // Check the requests
        Assert.assertTrue(listener.rcvRequests.contains(12));
        Assert.assertTrue(listener.rcvRequests.contains(22));
        Assert.assertTrue(listener.rcvRequests.contains(32));
        ...
        listener.rcvMessages.clear();

        sendBuffer.putInt(0, 11);
        IPC_PUBLISHER.sendMessage(MsgType.DATA, topicId1, sendBuffer, 0, 4);
        UCAST_PUBLISHER.sendMessage(MsgType.DATA, topicId1, sendBuffer, 0, 4);
        MCAST_PUBLISHER.sendMessage(MsgType.DATA, topicId1, sendBuffer, 0, 4);

        Thread.sleep(100);

        Assert.assertFalse(listener.rcvMessages.contains(11));
    ...
    public void pollMessagesOfMultipleSizesAndAssemble() throws Exception
    {
        ...
        randomMessages.forEach((msg) ->
        {
            ...
            UCAST_PUBLISHER.sendMessage(MsgType.DATA, UUID.randomUUID(), sendBuffer, 0, msg.length);
        });
        ...
        Assert.assertTrue(listener.rcvMessages.size() == randomMessages.size());
        ...
        for (int i = 0; i < randomMessages.size(); i++)
        {
            ...
            // Check length
            Assert.assertTrue(receivedMsg.getContentLength() == randomMessages.get(i).length);
    ...
    {
        @Getter final Set<Integer> rcvMessages = new HashSet<>();
        @Getter final AtomicInteger rcvEncryptedMessagesCount = new AtomicInteger(0);
        @Getter final Set<Integer>  rcvRequests = new HashSet<>();
        ...
        {
            rcvMessages.add(msg.getContents().getInt(msg.getContentOffset()));
        }
        ...
        {
            rcvRequests.add(request.getContents().getInt(request.getContentOffset()));
