...
public final class VegaInstance implements IVegaInstance, AvailableImageHandler, UnavailableImageHandler, ErrorHandler
{
    ...
    private VegaInstance(final VegaInstanceParams parameters) throws VegaException
    {
        ...
        // Start the embedded daemon if required, use the unmanaged one or the external
        ...
        else if (config.getDriverType() == AeronDriverType.EMBEDDED)
        {
            this.embeddedMediaDriver = new EmbeddedMediaDriver(config.getEmbeddedDriverConfigFile());
            aeronContext.aeronDirectoryName(this.embeddedMediaDriver.getDriverDirectoryName());
        }
        else if (config.getDriverType() == AeronDriverType.LOWLATENCY_EMBEDDED)
        {
            this.embeddedMediaDriver = new EmbeddedLowLatencyMediaDriver(config.getEmbeddedDriverConfigFile());
            aeronContext.aeronDirectoryName(this.embeddedMediaDriver.getDriverDirectoryName());
        }
        else
        {
