...
class CommandLineParser
{
    ...
    /** True to use an embedded low latency aeron driver */
    private final Option embeddedLowLatencyDriverOption  = new Option("edll", "embeddedDriverLowLatency", false, "(Optional) Set to true to use an embedded low latency driver");
    /** External driver directory path */
    ...
    private void addCommonCommandLineOptions(final Options options)
    {
        ...
        options.addOption(this.embeddedLowLatencyDriverOption);
    ...
    private DaemonParameters parseAndValidateCommandLine() throws AutodiscException
    {
        ...
        final boolean useLowLatencyEmbeddedDriver = this.hasOption(this.embeddedLowLatencyDriverOption);
        final String embeddedDriverConfigString = this.getCmdStringOption(this.embeddedDriverConfigFileOption);

        // Check that both drivers are not selected
        if (useEmbeddedDriver && useLowLatencyEmbeddedDriver)
        {
            throw new AutodiscException("Cannot select both use embedded driver and use low latency embedded driver options, choose only one");
        }
        ...
        else if (useLowLatencyEmbeddedDriver)
        {
            aeronDriverType = DaemonParameters.AeronDriverType.LOWLATENCY_EMBEDDED;
        }
        else
        {
